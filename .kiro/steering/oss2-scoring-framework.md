...
## What Changed (OSS → SDS/SPS/Pareto)
...
2. **Pareto analysis on SDS vs SPS** is the primary comparison tool. Dominated schedulers are eliminated; frontier schedulers represent genuine tradeoffs.
3. **No composite OSS.** The single-number score is removed entirely. SDS and SPS are reported independently and compared via Pareto dominance.
...
4. Drill down into individual metrics to explain results.
...
- Returns: per-metric normalised scores, SDS, SPS, Pareto frontier, tradeoff ratios
